...
class map_test2() :
  # Constructor
  def __init__(self,states,sDate, fDate, rates):
        # self.sDate = sDate
        # self.fDate = fDate
        # self.rates = rates
        # self.mapHash = {}
  ...
  def main( self ) :
    ...
    options = [ self.states, self.sDate,
    ...
    if self.mapHash.get( key ) == None :
      ...
      # while self.mapHash.get ( key ) == None :
        ...
        with csv_generator_lock :
          ...
          if key not in csvHash:
            ...
            with open( os.path.abspath('./cowiz/usmap/csvLayers/panel3DataGenerator.conf'), 'r' ) as fp :
            ...
            temp = str(options[0])
            allLines[1] = (self.fixDate(options[1])+'\r\n') # Update new start date
            allLines[3] = (self.fixDate(options[2])+'\r\n') # Update new second date
            allLines[5] = (str(options[3])) # Update new features choice 

            #print(allLines)

            time.sleep(5)

            with open( os.path.abspath('./cowiz/usmap/csvLayers/panel3DataGenerator.conf'), 'w' ) as fp :
              fp.writelines(allLines)
            fp.close()

            subprocess.call([ os.path.abspath('./cowiz/usmap/csvLayers/panel3csvGenerator') ], 
      ...
      with map_generator_lock_lock :
        ...
        with map_generator_locks[key] :
          ...
          mapPath = f"{ os.path.abspath('./cowiz/static/maps') }/{ subName }"
          ...
          with open( os.path.abspath('./cowiz/usmap/maptest/tables/html_table.txt'), 'a' ) as fp :
            if not success :
...
#-------------------------------------------------------------
  ...
  def generateMap( self, dirName, fName ) :
    ...
    """
    #path = ( './cowiz/usmap/csvLayers/'+ dirName + '/' + fName + '.csv' )
    # ASantra: changed path to the location where csv files get generated
    path = f"{ os.path.abspath('./cowiz/usmap/csvLayers') }/{ fName }.csv"
    ...
    nDir  = ( os.path.abspath('./cowiz/static/maps/') + dirName + '/' )
    ...
    fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0},
                      coloraxis_colorbar = dict(
                        ...
                        ticks = 'inside',
                        tickvals = [1, 2, 3],
                        ticktext = ['<b>New cases<b><br>-100%', '<b>New deaths<b><br>(-100%,50%]', 
                                    '<b>Vaccination<b><br>(-50%, 0%)'],
                        tickfont = dict(
                          size = 9),
                        thickness = 35,
                        xpad = 5,
...
#-------------------------------------------------------------
  ...
  def initCSVHash( self ) :
    ...
    open( os.path.abspath('./cowiz/usmap/maptest/tables/csv_table.txt'), 'w+' ).close()
    ...
    for path, subd, files in os.walk( os.path.abspath('./cowiz/usmap/csvLayers/') ) :
      for names in files :
        ...
        if (names[-3:] == 'csv'):
          ...
          if (len(keys) == 10) and (keys[-1] == 'csv'):
            ...
            with open( os.path.abspath('./cowiz/usmap/maptest/tables/csv_table.txt'), 'a+' ) as fp :
              fp.write( key + ',' + names + '\r\n' )
    ...
    with csv_rewriter_lock:
      with open( os.path.abspath('./cowiz/usmap/maptest/tables/csv_table.txt'), 'r' ) as fp :
...
#-------------------------------------------------------------
  ...
  def initMapHash( self ) : 
    ...
    with open( os.path.abspath('./cowiz/usmap/maptest/tables/html_table.txt'), 'r' ) as fp :
      lines = fp.read().replace( '\r', '' ).split( '\n' )
...
#test = map_test( 3, '04-JUL-21', '04-AUG-21', 3 )
#key = test.main()
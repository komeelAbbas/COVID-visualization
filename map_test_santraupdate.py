...
class map_test() :
  ...
  #def __init__(self, 1, '01-FEB-22', '02-FEB-22', 1 ):
      ...
      # while self.mapHash.get ( key ) == None :
        ...
        with csv_generator_lock :
          ...
          if key not in csvHash:
            ...
            with open( '/var/www/html/cowiz-test/cowiz/usmap/csvLayers/Covid19Period.conf', 'r' ) as fp :
            ...
            with open( '/var/www/html/cowiz-test/cowiz/usmap/csvLayers/Covid19Period.conf', 'w' ) as fp :
              fp.writelines(allLines)
            fp.close()

            subprocess.call(["/var/www/html/cowiz-test/cowiz/usmap/csvLayers/csvGenerator"], 
        ...
        with map_generator_lock_lock :
          ...
          with map_generator_locks[key] :
            ...
            mapPath = '/var/www/html/cowiz-test/cowiz/static/maps/'+ subName
            ...
            with open( '/var/www/html/cowiz-test/cowiz/usmap/maptest/tables/html_table.txt', 'a' ) as fp :
              if not success :
...
#-------------------------------------------------------------
  ...
  def generateMap( self, dirName, fName ) :
    ...
    """
    #path = ( '/var/www/html/cowiz-test/cowiz/usmap/csvLayers/'+ dirName + '/' + fName + '.csv' )
    # ASantra: changed path to the location where csv files get generated
    path = ( '/var/www/html/cowiz-test/cowiz/usmap/csvLayers/'+ fName + '.csv' )
    ...
    nDir  = ( '/var/www/html/cowiz-test/cowiz/static/maps/' + dirName + '/' )
...
#-------------------------------------------------------------
  ...
  def initCSVHash( self ) :
    ...
    open( '/var/www/html/cowiz-test/cowiz/usmap/maptest/tables/csv_table.txt', 'w+' ).close()
    ...
    for path, subd, files in os.walk( '/var/www/html/cowiz-test/cowiz/usmap/csvLayers/' ) :
      for names in files :
        ...
        if (names[-3:] == 'csv'):
          ...
          if (len(keys) == 10) and (keys[-1] == 'csv'):
            ...
            with open( '/var/www/html/cowiz-test/cowiz/usmap/maptest/tables/csv_table.txt', 'a+' ) as fp :
              fp.write( key + ',' + names + '\r\n' )
    ...
    with csv_rewriter_lock:
      with open( '/var/www/html/cowiz-test/cowiz/usmap/maptest/tables/csv_table.txt', 'r' ) as fp :
...
#-------------------------------------------------------------
  ...
  def initMapHash( self ) : 
    ...
    with open( '/var/www/html/cowiz-test/cowiz/usmap/maptest/tables/html_table.txt', 'r' ) as fp :
      lines = fp.read().replace( '\r', '' ).split( '\n' )
